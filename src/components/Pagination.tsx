      ...
      {total <= 10 ? (
        ...
        <>
          {parseInt(page) > 1 && (
            ...
            >
              <span className='px-3 py-2 rounded border shadow-sm bg-white cursor-pointer'>
                이전
              </span>
            </Link>
          )}
          <Link href={{ pathname: '/stores', query: { page: parseInt(page) } }}>
            <span className='px-3 py-2 rounded border shadow-sm bg-white text-blue-500 font-bold'>
              {page}
            </span>
          </Link>
          {total > parseInt(page) && (
